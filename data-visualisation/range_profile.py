...
subjects = data['subjects']  # Assuming 'subjects' contains the subject names like 'astitva', 'pranil', 'rishi'import numpy as np
import matplotlib.pyplot as plt

# # Load processed data
# data = np.load('processed_data.npz')
rangeProfile_data = data['features_1']  # Assuming 'features_1' is the range profile with shape (num_samples, 256)
# labels = data['labels']
...
for class_index, class_name in enumerate(class_names):
    for subject_index, subject_name in enumerate(subject_names):
        ...
        range_profile = rangeProfile_data[sample_index]
...
# Adjust layout
plt.tight_layout()
plt.savefig("range_profile_visualization.png")
plt.show()
